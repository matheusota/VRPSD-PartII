...
inline myutilsdefaultparameters::myutilsdefaultparameters() {
    pdfreader = "okular"; // Program to open a pdf file in Mac OS X
...
inline void set_MY_EPS(double neweps) {
    *(myutilsparameters.P_MY_EPS) = neweps;
}
inline void set_MY_INF(double newinf) {
    *(myutilsparameters.P_MY_INF) = newinf;
}
...
inline bool IsSuffix(string str, string suf) {
    return (str.size() >= suf.size() &&
            !str.compare(str.size() - suf.size(), suf.size(), suf));
}
...
inline int hex2int(string s) {
    ...
    for (int i = 0; i < n; i++) {
        ...
        if ((c >= '0') && (c <= '9'))
            ...
            cout << "Error: character \"" << s[i] << "\" in string \"" << s
...
void printtime(
    long t); /* print the time t in days, hours, minutes and seconds.*/
void sprinttime(char *s, long t); /* prints the time in the string s Example: 1
                                     hour, 2 minutes, 3 seconds*/
void shortprinttime(
    long t); /* prints the time in the string s. Ex.: 11d,22h:33m:44s   */
...
class StringTable {
  public:
    ...
    int columnindex(
        string columnname); // return column index, or -1 if it does not exists
    string first(string columnname); // return the first element of the column
    int firstint(
        string columnname); // return the first element of the column as int
    double firstdouble(
        string columnname); // return the first element of the column as double
