...
typedef Graph::NodeMap<std::vector<int>> NodeVectorIntMap;
typedef Graph::NodeMap<std::vector<double>> NodeVectorValueMap;
...
typedef Graph::NodeMap<GRBVar> NodeGRBVarMap;
typedef Graph::NodeMap<std::vector<GRBVar>> NodeVectorGRBVarMap;
...
typedef Digraph::ArcMap<GRBVar> ArcGRBVarMap;
...
bool InitGraphTable(StringTable &T, Graph &g, StringToNodeMap &smap,
                    string colname_v1, string colname_v2,
...
bool ReadGraphColumn(StringTable &T, LineToNodeMap &nodemap, string colname,
                     NodeValueMap &read);
bool ReadGraphColumn(StringTable &T, LineToNodeMap &nodemap, string colname,
                     NodeStringMap &read);
bool ReadGraphColumn(StringTable &T, LineToNodeMap &nodemap, string colname,
                     NodeIntMap &read);
bool ReadGraphColumn(StringTable &T, LineToEdgeMap &edgemap, string colname,
                     EdgeValueMap &read);
bool ReadGraphColumn(StringTable &T, LineToEdgeMap &edgemap, string colname,
                     EdgeStringMap &read);
bool ReadGraphColumn(StringTable &T, LineToEdgeMap &edgemap, string colname,
                     EdgeIntMap &read);
bool ReadGraphColumn(StringTable &T, LineToDNodeMap &nodemap, string colname,
                     DNodeValueMap &read);
bool ReadGraphColumn(StringTable &T, LineToDNodeMap &nodemap, string colname,
                     DNodeStringMap &read);
bool ReadGraphColumn(StringTable &T, LineToDNodeMap &nodemap, string colname,
                     DNodeIntMap &read);
bool ReadGraphColumn(StringTable &T, LineToArcMap &arcmap, string colname,
                     ArcValueMap &read);
bool ReadGraphColumn(StringTable &T, LineToArcMap &arcmap, string colname,
                     ArcStringMap &read);
bool ReadGraphColumn(StringTable &T, LineToArcMap &arcmap, string colname,
...
bool GenerateVertexPositions(Graph &g, EdgeValueMap &custo, NodePosMap &posx,
                             NodePosMap &posy);
bool GenerateVertexPositions(Digraph &g, ArcValueMap &custo, DNodePosMap &posx,
                             DNodePosMap &posy);
...
class AdjacencyMatrix {
  public:
    AdjacencyMatrix(Graph &graph, EdgeValueMap &graphweight,
                    double NonEdgeValue);
    ~AdjacencyMatrix();
    double *AdjMatrix;
    Graph *g;
...
bool GenerateRandomEuclideanGraph(
    Graph &g,
...
bool GenerateRandomEuclideanDigraph(
    Digraph &g,
...
// vector: nodes v in the same side of s have cut[v]=true, otherwise
// cut[v]=false.
double DiMinCut(Digraph &g, ArcValueMap &weight, DNode &s, DNode &t,
                DCutMap &cut);
...
//      nodename
//      endpoint1 endpoint2
// the other fields are optional. It depends on the informations you want to
// obtain. For some problems, there is only the edge weight. In other, there is
// node weight,...
class GraphTable {
  public:
    ...
    bool GetNodeCoordinates(string colx, NodePosMap &posx, string coly,
                            NodePosMap &posy);
    void print();
...
inline GraphTable::GraphTable(string filename, Graph &graph) : g(graph) {
    ...
    if (type == "graph") {
        ...
        InitGraphTable(*(this->EdgeTable), this->g, this->name2node,
                       "endpoint1", "endpoint2", this->line2edge);
    }
...
inline int GraphTable::getNNodes() { return this->n; }

inline int GraphTable::getNEdges() { return this->m; }
...
//      nodename
//      endpoint1 endpoint2
// the other fields are optional. It depends the informations you want to
// obtain. For some problems, there is only the edge weight. In other, there is
// node weight,...
class DigraphTable {
  public:
...
inline DigraphTable::DigraphTable(string filename, Digraph &graph) : g(graph) {
    ...
    if (type == "digraph") {
        ...
        InitGraphTable(*(this->ArcTable), this->g, this->name2node, "tail",
                       "head", this->line2arc);
    }
    file.close();
}
...
bool GetNodeCoordinates(DigraphTable &GT, string colx, DNodePosMap &posx,
                        string coly, DNodePosMap &posy);

//--------------------------------------------------------------------------------
class GraphAttributes {
  public:
    GraphAttributes(const Graph &graph, const NodeStringMap &vname,
                    const NodePosMap &posx, const NodePosMap &posy);
    ...
    void SetColorByInterval(double lb, double ub, NodeValueMap &nodeval,
                            string cor);
    void SetColorByValue(double value, EdgeValueMap &edgeval, string cor);
    void SetColorByInterval(double lb, double ub, EdgeValueMap &edgeval,
                            string cor);
    ...
    void SetAttribByInterval(double lb, double ub, NodeValueMap &nodeval,
                             string at);
    ...
    void SetAttribByInterval(double lb, double ub, EdgeValueMap &edgeval,
                             string at);

    bool Save(string name);
    bool View();
    bool View(NodeValueMap &weights);
};

inline void GraphAttributes::SetImageFactor(double f) { this->factor = f; }

inline void GraphAttributes::SetGraphAttrib(string s) {
    this->graph_attrib += " " + s;
}

inline void GraphAttributes::SetDefaultNodeAttrib(string s) {
    this->default_node_attrib += " " + s;
}
inline void GraphAttributes::SetDefaultEdgeAttrib(string s) {
    this->default_edge_attrib += " " + s;
}
inline void GraphAttributes::SetAttrib(Node v, string s) {
    this->node_attrib[v] += " " + s;
}
inline void GraphAttributes::SetAttrib(Edge e, string s) {
    this->edge_attrib[e] += " " + s;
}

inline void GraphAttributes::SetShape(Node v, string s) {
    this->SetAttrib(v, "shape=\"" + s + "\"");
}
inline void GraphAttributes::SetShape(Edge e, string s) {
    this->SetAttrib(e, "shape=\"" + s + "\"");
}

inline void GraphAttributes::SetStyle(Node v, string s) {
    this->SetAttrib(v, "style=\"" + s + "\"");
}
inline void GraphAttributes::SetStyle(Edge e, string s) {
    this->SetAttrib(e, "style=\"" + s + "\"");
}
...
inline void GraphAttributes::SetLabel(Node v, string s) {
    this->SetAttrib(v, "label=\"" + s + "\"");
}
inline void GraphAttributes::SetLabel(Edge e, string s) {
    this->SetAttrib(e, "label=\"" + s + "\"");
}
inline void GraphAttributes::SetLabel(Node v, double d) {
    this->SetLabel(v, DoubleToString(d));
}
inline void GraphAttributes::SetLabel(Edge e, double d) {
    this->SetLabel(e, DoubleToString(d));
}
inline void GraphAttributes::SetLabel(string s) {
    this->SetGraphAttrib("label=\"" + s + "\"");
}
...
inline void GraphAttributes::SetDefaultNodeFontName(string s) {
    this->SetDefaultNodeAttrib("fontname=\"" + s + "\"");
}
inline void GraphAttributes::SetDefaultEdgeFontName(string s) {
    this->SetDefaultEdgeAttrib("fontname=\"" + s + "\"");
}
inline void GraphAttributes::SetFontName(Node v, string s) {
    this->SetAttrib(v, "fontname=\"" + s + "\"");
}
inline void GraphAttributes::SetFontName(Edge e, string s) {
    this->SetAttrib(e, "fontname=\"" + s + "\"");
}

inline void GraphAttributes::SetDefaultNodeFontSize(double s) {
    this->SetDefaultNodeAttrib("fontsize=\"" + DoubleToString(s) + "\"");
}
inline void GraphAttributes::SetDefaultEdgeFontSize(double s) {
    this->SetDefaultEdgeAttrib("fontsize=\"" + DoubleToString(s) + "\"");
}
inline void GraphAttributes::SetFontSize(Node v, double s) {
    this->SetAttrib(v, "fontsize=\"" + DoubleToString(s) + "\"");
}
inline void GraphAttributes::SetFontSize(Edge e, double s) {
    this->SetAttrib(e, "fontsize=\"" + DoubleToString(s) + "\"");
}
inline void GraphAttributes::SetFontSize(double s) {
    this->SetGraphAttrib("fontsize=\"" + DoubleToString(s) + "\"");
}

inline void GraphAttributes::SetColorByValue(int value, NodeIntMap &nodeval,
                                             string cor) {
...
inline void GraphAttributes::SetAttribByValue(int value, NodeIntMap &nodeval,
                                              string at) {
...
inline void GraphAttributes::SetAttribByValue(int value, EdgeIntMap &edgeval,
                                              string at) {
    for (EdgeIt e(this->g); e != INVALID; ++e)
        if (value == edgeval[e])
            this->SetAttrib(e, at);
...
class DigraphAttributes {
  public:
    ...
    void SetColorByInterval(double lb, double ub, DNodeValueMap &nodeval,
                            string cor);
    void SetColorByInterval(double lb, double ub, ArcValueMap &arcval,
                            string cor);
    ...
    void SetAttribByInterval(double lb, double ub, DNodeValueMap &nodeval,
                             string at);
    void SetAttribByInterval(double lb, double ub, ArcValueMap &arcval,
                             string at);

    bool Save(string name);
    bool View(); // text displayed below the figure
    bool View(int aux_factor);
};
inline void DigraphAttributes::SetImageFactor(double f) { this->factor = f; }

inline void DigraphAttributes::SetDigraphAttrib(string s) {
    this->digraph_attrib += " " + s;
}

inline void DigraphAttributes::SetDefaultDNodeAttrib(string s) {
    this->default_node_attrib += " " + s;
}
inline void DigraphAttributes::SetDefaultArcAttrib(string s) {
    this->default_arc_attrib += " " + s;
}
inline void DigraphAttributes::SetAttrib(DNode v, string s) {
    this->node_attrib[v] += " " + s;
}
inline void DigraphAttributes::SetAttrib(Arc e, string s) {
    this->arc_attrib[e] += " " + s;
}

inline void DigraphAttributes::SetShape(DNode v, string s) {
    this->SetAttrib(v, "shape=\"" + s + "\"");
}
inline void DigraphAttributes::SetShape(Arc e, string s) {
    this->SetAttrib(e, "shape=\"" + s + "\"");
}

inline void DigraphAttributes::SetStyle(DNode v, string s) {
    this->SetAttrib(v, "style=\"" + s + "\"");
}
inline void DigraphAttributes::SetStyle(Arc e, string s) {
    this->SetAttrib(e, "style=\"" + s + "\"");
}
...
inline void DigraphAttributes::SetLabel(DNode v, string s) {
    this->SetAttrib(v, "label=\"" + s + "\"");
}
inline void DigraphAttributes::SetLabel(Arc e, string s) {
    this->SetAttrib(e, "label=\"" + s + "\"");
}
inline void DigraphAttributes::SetLabel(DNode v, double d) {
    this->SetLabel(v, DoubleToString(d));
}
inline void DigraphAttributes::SetLabel(Arc e, double d) {
    this->SetLabel(e, DoubleToString(d));
}
inline void DigraphAttributes::SetLabel(string s) {
    this->SetDigraphAttrib("label=\"" + s + "\"");
}
...
inline void DigraphAttributes::SetDefaultArcFontName(string s) {
    this->SetDefaultArcAttrib("fontname=\"" + s + "\"");
}
inline void DigraphAttributes::SetFontName(DNode v, string s) {
    this->SetAttrib(v, "fontname=\"" + s + "\"");
}
inline void DigraphAttributes::SetFontName(Arc e, string s) {
    this->SetAttrib(e, "fontname=\"" + s + "\"");
}

inline void DigraphAttributes::SetDefaultDNodeFontSize(double s) {
    this->SetDefaultDNodeAttrib("fontsize=\"" + DoubleToString(s) + "\"");
}
inline void DigraphAttributes::SetDefaultArcFontSize(double s) {
    this->SetDefaultArcAttrib("fontsize=\"" + DoubleToString(s) + "\"");
}
inline void DigraphAttributes::SetFontSize(DNode v, double s) {
    this->SetAttrib(v, "fontsize=\"" + DoubleToString(s) + "\"");
}
inline void DigraphAttributes::SetFontSize(Arc e, double s) {
    this->SetAttrib(e, "fontsize=\"" + DoubleToString(s) + "\"");
}
inline void DigraphAttributes::SetFontSize(double s) {
    this->SetDigraphAttrib("fontsize=\"" + DoubleToString(s) + "\"");
}

// ----
inline void DigraphAttributes::SetColorByValue(double value,
                                               DNodeValueMap &nodeval,
                                               string cor) {
    for (DNodeIt v(this->g); v != INVALID; ++v)
        if (IsEqual(value, nodeval[v]))
            this->SetColor(v, cor);
...
inline void DigraphAttributes::SetColorByInterval(double lb, double ub,
                                                  DNodeValueMap &nodeval,
                                                  string cor) {
    for (DNodeIt v(this->g); v != INVALID; ++v)
        if ((nodeval[v] > lb - MY_EPS) && (nodeval[v] < ub + MY_EPS))
...
// ----
inline void DigraphAttributes::SetAttribByValue(double value,
                                                DNodeValueMap &nodeval,
                                                string at) {
    for (DNodeIt v(this->g); v != INVALID; ++v)
        if (IsEqual(value, nodeval[v]))
...
inline void DigraphAttributes::SetAttribByValue(int value, DNodeIntMap &nodeval,
                                                string at) {
    for (DNodeIt v(this->g); v != INVALID; ++v)
        if (value == nodeval[v])
            this->SetAttrib(v, at);
...
inline void DigraphAttributes::SetAttribByInterval(double lb, double ub,
                                                   DNodeValueMap &nodeval,
                                                   string at) {
    for (DNodeIt v(this->g); v != INVALID; ++v)
        if ((nodeval[v] > lb - MY_EPS) && (nodeval[v] < ub + MY_EPS))
...
inline void DigraphAttributes::SetAttribByValue(int value, ArcIntMap &arcval,
                                                string at) {
    for (ArcIt e(this->g); e != INVALID; ++e)
        if (value == arcval[e])
            this->SetAttrib(e, at);
...
bool Graph2Digraph(Graph &g, Digraph &dg, NodeDNodeMap &nodemap,
                   EdgeArcMap &edgemap1, EdgeArcMap &edgemap2);
