...
class ColorClass {
  public:
    ...
    // ColorClass(string filename);//Add color from a file (also includes the
    // colors in init) some functions
...
inline string ColorRGB(string colorname) {
    return (ColorG.rgb(ColorG.code[colorname]));
}
...
inline int ColorInsert(string colorname, string rgb) {
    return (ColorG.insert(colorname, rgb));
}
...
inline string ith_VisualDistinctColorName(int i) {
    return (ColorG.name(ith_VisualDistinctColor(i)));
}
#endif
