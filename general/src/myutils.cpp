...
long time70(void) { return ((long)time(0)); }

// given a time in seconds, print corresponding time in days, hours, minutes and
// seconds
...
// given a time in seconds, generate a string containing the time in days,
...
StringTable::StringTable(int nr, ifstream &file) {
    ...
    for (int i = 0; i < nrows; i++) {
        ...
        while (getline(token, word, ' ')) {
            ...
            if (c == this->ncols) {
                cout << i + 1 << ": \"" << h << "\"" << endl;
                cout << "Number of cols, in line " << i + 1
                     << " is larger than in header." << endl;
                exit(1);
            }
            this->line[i][c] = word;
        ...
        if (c < this->ncols) {
            cout << "Number of cols, in line " << i + 1
                 << " is smaller than in header." << endl;
            exit(1);
        }
    }
    return;
...
string StringTable::first(string columnname) {
    ...
    if (col != -1)
        ...
        cout << "Table does not contain column name \"" << columnname << "\"."
             << endl;
        exit(1);
    }
}
