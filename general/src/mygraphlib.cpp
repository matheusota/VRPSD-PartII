...
Node GetNodeByName(Graph &g,
    ...
    cout << "GetNodeByName: Error to obtain vertex node \"" << vertexname
         << "\"" << endl;
    exit(0);
}
...
bool GenerateVertexPositions(Graph &g, EdgeValueMap &custo, NodePosMap &posx,
    ...
    out << "}\n";
    out.close();
    sprintf(cmd, "%s -Goverlap=false %s -o %s",
            mygraphlibparameters.graphvizdrawingprogram.c_str(), tempnamedot,
            tempnameposdot);
    fflush(stdout);
    ...
    if (t == string::npos) {
        cout << "Temp Graphviz file is not appropriate for "
    ...
    for (NodeIt v(g); v != INVALID; ++v) {
        ...
        if (t != string::npos) {
            ...
            printf("GenerateVertexPositions: Error to obtain vertex "
                   "coordinates.\n");
            return (false);
        }
    }
...
bool GraphTable::GetNodeCoordinates(string colx, NodePosMap &posx, string coly,
                                    NodePosMap &posy) {
    // read from file
    if ((this->NodeTable->columnindex(colx) != -1) &&
        (this->NodeTable->columnindex(coly) != -1)) {
        this->GetColumn(colx, posx);
        this->GetColumn(coly, posy);
    }
    // no positions in the file, call a routine to create positions for the
    // nodes
    else
...
bool InitGraphTable(
    StringTable &T, Graph &g,
    LineToNodeMap &nodemap) { // Make a correspondence between row i and node v
...
bool InitGraphTable(
    StringTable &T, Graph &g, StringToNodeMap &smap, string colname_v1,
    string colname_v2,
...
bool InitGraphTable(StringTable &T, Digraph &g,
                    LineToDNodeMap &dnodemap) { // Make a correspondence between
                                                // row i and node v
...
bool InitGraphTable(
    StringTable &T, Digraph &g, StringToDNodeMap &smap, string colname_source,
    string colname_target,
    LineToArcMap &arcmap) { // Make a correspondence between row i and arc a
    arcmap.reserve(T.nrows);
    arcmap.resize(T.nrows);
    int col_source = T.columnindex(colname_source),
        col_target = T.columnindex(colname_target);
    // cout << "nrows = " << T.nrows << endl;
    // cout << "source and target: " << col_source << "   " << col_target <<
    // endl;
    string name_source, name_target;
    for (int i = 0; i < T.nrows; i++) {
        T.entry(i, col_source, name_source);
        T.entry(i, col_target, name_target);
        // cout << "namesource and nametarget: " << name_source << "   " <<
        // name_target << endl;
        DNode source = smap[name_source], target = smap[name_target];
        // cout << "sourceid and targetid: " << g.id(source) << "   " <<
        // g.id(target) << endl;
...
bool ReadGraph(string filename, Graph &g, NodeStringMap &vname,
    ...
    bool ok =
        GT.GetColumn(mygraphlibparameters.inputgraphtable_nodename, vname);
    ok = ok && GT.GetColumn("weight", weight);
    ok = ok &&
         GT.GetNodeCoordinates(mygraphlibparameters.inputgraphtable_posx, posx,
    ...
    double SizeY)          // coordinate y is a random number in [0,SizeY)
{
    ...
    V = new DNode[n];
    if (V == NULL) {
        cout << "Memory allocation error, number of nodes " << n
             << " too large\n";
        exit(0);
    }

    for (int i = 0; i < n;
         i++) { // insert nodes (random points in [0,100] x [0,100] )
    ...
    for (int i = 0; i < n; i++)
        for (int j = i + 1; j < n; j++) {
            ...
            weight[e] =
                sqrt(pow(px[V[i]] - px[V[j]], 2) + pow(py[V[i]] - py[V[j]], 2));
    ...
    double SizeY)         // coordinate y is a random number in [0,SizeY)
{
    ...
    double *p =
        new double[2 * n +
                   2]; // node coodinates are (x;y) = ( p[2*i] ; p[2*i+1] )
    ...
    for (i = 0; i < ntri; i++) {
        ...
        if ((findEdge(g, V[a], V[b]) == INVALID) &&
            (findEdge(g, V[b], V[a]) == INVALID)) {
            Edge e = g.addEdge(V[a], V[b]);
            weight[e] =
                sqrt(pow(px[V[a]] - px[V[b]], 2) + pow(py[V[a]] - py[V[b]], 2));
        }
        if ((findEdge(g, V[a], V[c]) == INVALID) &&
            (findEdge(g, V[c], V[a]) == INVALID)) {
            Edge e = g.addEdge(V[a], V[c]);
            weight[e] =
                sqrt(pow(px[V[a]] - px[V[c]], 2) + pow(py[V[a]] - py[V[c]], 2));
        }
        if ((findEdge(g, V[b], V[c]) == INVALID) &&
            (findEdge(g, V[c], V[b]) == INVALID)) {
            Edge e = g.addEdge(V[b], V[c]);
            weight[e] =
                sqrt(pow(px[V[b]] - px[V[c]], 2) + pow(py[V[b]] - py[V[c]], 2));
        }
    }
    delete[] V;
    delete[] p;
    ...
    double SizeY)          // coordinate y is a random number in [0,SizeY)
{
    ...
    for (i = 0; i < n; i++) { // insere os vértices (pontos aleatórios no plano
                              // [0,SizeX]x[0,SizeY] )
        V[i] = g.addNode();   // gera um vértice nó do grafo
        px[V[i]] = SizeX * drand48(); // nodes are random points
        py[V[i]] = SizeY * drand48();
    ...
    for (i = 0; i < ntri; i++) {
        ...
        if (findArc(g, V[b], V[a]) == INVALID) {
            ...
            weight[e] =
                sqrt(pow(px[V[b]] - px[V[a]], 2) + pow(py[V[b]] - py[V[a]], 2));
        }
        ...
        if (findArc(g, V[c], V[a]) == INVALID) {
            ...
            weight[e] =
                sqrt(pow(px[V[c]] - px[V[a]], 2) + pow(py[V[c]] - py[V[a]], 2));
        }
        ...
        if (findArc(g, V[c], V[b]) == INVALID) {
            ...
            weight[e] =
                sqrt(pow(px[V[c]] - px[V[b]], 2) + pow(py[V[c]] - py[V[b]], 2));
        }
    }
    delete[] V;
    delete[] p;
...
void PrintGraph(Graph &g, NodeStringMap &vname, EdgeValueMap &graphweight) {
    ...
    printf("-------------------------------------------------------\n");
    for (EdgeIt a(g); a != INVALID; ++a)
        printf("%s -- %s  %lf\n", vname[g.u(a)].c_str(), vname[g.v(a)].c_str(),
               graphweight[a]);
    printf("\n");
}

GraphAttributes::GraphAttributes(const Graph &graph, const NodeStringMap &vname,
                                 const NodePosMap &posx, const NodePosMap &posy)
    : g(graph), node_name(graph), px(graph), py(graph), node_attrib(graph),
      edge_attrib(graph) {
    factor = 1.0;
    graph_attrib = "";
...
GraphAttributes::GraphAttributes(GraphTable &GT)
    : g(GT.g), node_name(GT.g), px(GT.g), py(GT.g), node_attrib(GT.g),
      edge_attrib(GT.g) {
    factor = 1.0;
    graph_attrib = "";
...
bool GraphAttributes::View(
    NodeValueMap &weights) // text displayed below the figure
{
    char tempname[1000], cmd[1000], outputname[1000];
    FILE *fp;
    string str;
    double minpx = DBL_MAX, minpy = DBL_MAX, maxpx = -DBL_MAX, maxpy = -DBL_MAX,
           delta, normalfactor;
    ...
    normalfactor =
        100 / this->factor; // quanto maior, menor o desenho do vértice
    ...
    double minW = 99999;
    for (NodeIt v(this->g); v != INVALID; ++v) {
        maxW = max(maxW, weights[v]);
        minW = min(minW, weights[v]);
    }
    ...
    for (NodeIt v(this->g); v != INVALID; ++v) {
        ...
        fprintf(fp, "pos=\"%lf,%lf!\" ",
                normalfactor * (this->px[v] - minpx) / delta,
                normalfactor * (this->py[v] - minpy) / delta);
        fprintf(fp, "width=\"%lf\" ",
                5 * ((weights[v] - minW) / (maxW - minW)) + 1);
        fprintf(fp, " ];\n");
    }
    for (EdgeIt e(this->g); e != INVALID; ++e) {
        if (IsSuffix(this->edge_attrib[e], "color=\"Invis\""))
            continue; // only to speed graphviz exec.
        fprintf(fp, "\t%s  -- %s [", this->node_name[this->g.u(e)].c_str(),
                this->node_name[this->g.v(e)].c_str());
    ...
    fprintf(fp, "}\n");
    fclose(fp);
    sprintf(cmd, "%s -Tpdf %s -o %s",
            mygraphlibparameters.graphvizdrawingprogram.c_str(), tempname,
            outputname);
    system(cmd);
...
bool GraphAttributes::View() // text displayed below the figure
{
    ...
    normalfactor =
        40 / this->factor; // quanto maior, menor o desenho do vértice
    delta = fmax(maxpx - minpx, maxpy - minpy);
    // Generate a text file with the graph format of graphviz program
    fprintf(fp, "graph g {\n");
    fprintf(fp, "\t %s \n", this->graph_attrib.c_str()); // put graph attributes
...
DigraphAttributes::DigraphAttributes(Digraph &digraph, DNodeStringMap &vname,
    ...
    : g(digraph), node_name(vname), px(posx), py(posy), node_attrib(digraph),
      arc_attrib(digraph) {
    factor = 1.0;
    digraph_attrib = "";
...
bool DigraphAttributes::Save(string name) // text displayed below the figure
{
    char tempname[1000], cmd[1000], outputname[1000];
    FILE *fp;
    string str;
...
// graphviz.org) to generate a pdf file and a program to view the pdf file. The
// pdf viewer name is given in the viewername parameter.
bool DigraphAttributes::View(int aux_factor) // text displayed below the figure
{
    char tempname[1000], cmd[1000], outputname[1000];
    FILE *fp;
    string str;
    ...
    normalfactor =
        aux_factor / this->factor; // quanto maior, menor o desenho do vértice
    delta = fmax(maxpx - minpx, maxpy - minpy);
    // Generate a text file with the graph format of graphviz program
    fprintf(fp, "digraph g {\n");
    // fprintf(fp,"\tsize = \"10, 10\";\n");
...
// Define an adjacency matrix, so as we have a fast access for the edges of a
// graph, given a pair of vertices. This is mainly used in the subroutine 2opt,
// for dense graphs. The adjacency matrix is stored in a strictly triangular
// inferior matrix.

AdjacencyMatrix::AdjacencyMatrix(Graph &graph, EdgeValueMap &graphweight,
                                 double NonEdgValue)
    ...
    Nmatrix = (Nnodes * (Nnodes - 1)) /
              2; // no. of edges/elem. in strict. triang. inf. matrix
...
bool GreedyColoring(Graph &g, int &ncolors, NodeIntMap &nodecolor) {
    ...
    vector<Node> listdegree(
        maxdg); // listdegree[d]: circular list of nodes with degree d
    ...
    for (int i = 0; i < n;
         i++) { // in each loop, we get one node of smallest degree
        Node v;
        int dv;
    ...
    int maxremoveddegree = Max(
        g,
        degree); // maximum degree (considering degree when node was removed).
    ...
    for (int i = 0; i < n; i++) {
        ...
        // for (int j=0;j<nadjcolors;j++) cout<<adjacentcolors[j]<<", ";
        // cout<<endl;

        {
            ...
            // cout << "Cor a ser atribuida ao vertice "<< vname[v]<<"
            // "<<color_v<<endl<<endl;
            nodecolor[v] = color_v;
        }
    }
