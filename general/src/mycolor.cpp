...
string ColorClass::name(int colorcode) {
    return (this->vecolor[colorcode].colorname);
}
...
int InsertVisualDistinctColor(string colorname) {
    ...
    if (ColorG.firstvisualdistinctcolor == -1) {
        ...
        ColorG.vecolor[colorcode].nextvisualdistinctcolor =
            ColorG.firstvisualdistinctcolor;
        ColorG.vecolor[ColorG.lastvisualdistinctcolor].nextvisualdistinctcolor =
            colorcode;
        ColorG.lastvisualdistinctcolor = colorcode;
    }
...
void ColorClass::init(void) {
    ...
    this->code["NoColor"] =
        -1; // This is a special code to represent no given color
    this->insert(
        "Aqua",
        "#00FFFF"); // In some cases, the color 0 (first color) is default
...
//   this->init();
//   file.open(filename);
//   if (!file){cout<<"Error: Could not open file
//   \""<<filename<<"\".\n";exit(0);} while(std::getline(file, line)) {
//     if (this->vecolor.size() >= this->MaxColors) // In the future, change to
//     resize
...
//   for (int
//   i=0;i<this->vecolor.size();i++)this->code[this->vecolor[i].colorname]=i;
// }
...
void ColorClass::list() {
    ...
    while (it != code.end()) {
        cout << setw(30) << it->first << setw(5) << it->second << setw(10)
             << ColorRGB(it->second) << endl;
        it++;
    }
}
...
void ColorClass::print() {
    ...
    for (int i = 0; i < this->vecolor.size(); i++) {
        std::cout << setw(5) << i << " " << setw(30)
                  << this->vecolor[i].colorname << setw(10)
                  << this->vecolor[i].rgb << endl;
    }
}
